# creating 'northwind_dw' database
...
SELECT `orders`.`id`,
    ...
    `order_details_status`.`id`,
    `order_details_status`.`status_name`
FROM `northwind`.`orders`
INNER JOIN `northwind`.`orders_status` ON  `orders`.`status_id` =  `orders_status`.`id`
RIGHT OUTER JOIN `northwind`.`order_details` ON `order_details`.`status_id` = `order_details_status`.`id`
...
JOIN northwind_dw.dim_customers ON fact_orders.customer_id = dim_customers.customer_key
